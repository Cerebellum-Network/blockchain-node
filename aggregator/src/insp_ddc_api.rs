...
use crate::aggregator::json::{PHDNodesTCAs, PHDBucketsTCAs};
...
pub fn get_g_collectors_nodes<AccountId, BlockNumber, CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId> >(
    ...
    for (node_key, node_params) in collectors {
        if check_grouping_collector(&node_params)
            .map_err(|_| ApiError::NodeRetrievalError)?
        {
...
/// Parameters:
/// - `cluster_id`: Cluster id of a cluster.
pub fn get_collectors_nodes<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
) -> Result<Vec<(NodePubKey, StorageNodeParams)>, ApiError> {
    let mut collectors = Vec::new();

    let nodes =
        CM::get_nodes(cluster_id).map_err(|_| ApiError::NodeRetrievalError)?;

    for node_pub_key in nodes {
        if let Ok(NodeParams::StorageParams(storage_params)) =
            NM::get_node_params(&node_pub_key)
        {
...
pub fn fetch_bucket_challenge_response<AccountId, BlockNum, CM: ClusterManager<AccountId, BlockNum> , NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    tcaa_id: TcaEra,
...
pub fn fetch_node_challenge_response<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    tcaa_id: TcaEra,
    collector_key: NodePubKey,
    node_key: NodePubKey,
...
pub fn fetch_bucket_aggregates<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    tcaa_id: TcaEra,
    collector_key: NodePubKey,
...
pub fn fetch_traversed_era_historical_document<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    ehd_id: EHDId,
    tree_node_id: u32,
...
/// - `tree_node_id` - merkle tree node identifier
/// - `tree_levels_count` - merkle tree levels to request
pub fn fetch_traversed_partial_historical_document<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    phd_id: PHDId,
    tree_node_id: u32,
    tree_levels_count: u32,
...
/// - `phd_id`: EHD id
pub fn get_ehd_root<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    ehd_id: EHDId,
) -> Result<aggregator_client::json::EHDTreeNode, ApiError> {
    fetch_traversed_era_historical_document::<AccountId, BlockNumber,CM, NM>(cluster_id, ehd_id, 1, 1)?
        .first()
...
/// - `cluster_id`: Cluster Id
/// - `phd_id`: PHD id
pub fn get_phd_root<AccountId, BlockNumber,CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    phd_id: PHDId,
) -> Result<aggregator_client::json::PHDTreeNode, ApiError> {
    fetch_traversed_partial_historical_document::<AccountId, BlockNumber,CM, NM>(cluster_id, phd_id, 1, 1)?
        .first()
...
pub fn fetch_inspection_receipts<AccountId, BlockNumber, CM: ClusterManager<AccountId, BlockNumber>, NM: NodeManager<AccountId>>(
    cluster_id: &ClusterId,
    ehd_id: EHDId,
