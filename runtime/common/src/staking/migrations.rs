    ...
    impl<T: Config> OnRuntimeUpgrade for MigrateToV1<T> {
        ...
        fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
            frame_support::ensure!(
				...
				"Expected v0 before upgrading to v1"
			);
        ...
        fn on_runtime_upgrade() -> frame_support::weights::Weight {
            if StorageVersion::<T>::get() == 0 {
                let count = Nominators::<T>::get().count();
                ...
                Nominators::<T>::translate::<T::AccountId, Vec<T::AccountId>, _, _>(
                    |key| key,
        ...
        fn post_upgrade(prev_state: Vec<u8>) -> Result<(), TryRuntimeError> {
            ...
            frame_support::ensure!(current_version == ObsoleteReleases::V1_0_0Ancient, "must upgrade");
            ensure!(
                current_version == on_chain_version,
                "after migration, the current_version and on_chain_version should be the same"
            );

            Nominations::<T>::iter().try_for_each(|(_id, bucket)| -> Result<(), &'static str> {
                ensure!(
				!bucket.is_removed,
