...
fn transfer_hash() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Example::transfer_hash(Origin::signed(1), hash.clone(), dest_chain,));
...
fn execute_remark_bad_origin() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Example::remark(Origin::signed(RELAYER_A), hash), DispatchError::BadOrigin);
...
fn transfer() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Example::transfer(Origin::signed(Bridge::account_id()), RELAYER_A, 10));
...
fn mint_erc721() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Example::mint_erc721(Origin::signed(bridge_id), recipient, token_id, metadata.clone()),
...
fn create_sucessful_transfer_proposal() {
	new_test_ext().execute_with(|| {
		...
		assert_events(vec![
			...
			Event::balances(balances::RawEvent::Transfer(Bridge::account_id(), RELAYER_A, 10)),
