...
fn test_contract_api() {
	...
	let messages = contract_meta.pointer("/spec/messages").unwrap().as_array().unwrap();
	...
	let selector_get_current_period_ms =
		from_hex(get_current_period_ms.get("selector").unwrap().as_str().unwrap()).unwrap();
	...
	let selector_finalize_metric_period =
		from_hex(finalize_metric_period.get("selector").unwrap().as_str().unwrap()).unwrap();
	...
	assert_eq!(REPORT_DDN_STATUS_SELECTOR.to_vec(), selector_report_ddn_status);
}
