...
fn cluster_protocol_activation_proposal_initiated() {
	...
	ExtBuilder.build_and_execute(cluster, vec![node_1, node_2, node_3], || {
		...
		let cluster_protocol_params = default_cluster_protocol_params::<Test>();
		...
		assert_eq!(votes, Some(Votes { seats, threshold, ayes: vec![], nays: vec![], start, end }));
...
fn cluster_protocol_activation_proposal_early_approved_with_supermajority_consensus_and_prime_default_vote(
	...
	ExtBuilder.build_and_execute(cluster, vec![node_1, node_2, node_3], || {
		...
		let referenda_count = polkadot_sdk::pallet_referenda::ReferendumCount::<Test>::get();
...
fn cluster_protocol_activation_proposal_early_disapproved_with_supermajority_consensus_and_prime_default_vote(
	...
	ExtBuilder.build_and_execute(cluster, vec![node_1, node_2, node_3], || {
		...
		assert_eq!(polkadot_sdk::pallet_referenda::ReferendumCount::<Test>::get(), 0);
