...
benchmarks! {
  ...
  verify {
	...
	set_cluster_params {
	  ...
	  let new_cluster_params = ClusterParams { node_provider_auth_contract: user_2.clone() };
	}: _(RawOrigin::Signed(user.clone()), cluster_id, new_cluster_params)
	verify {
	  assert_eq!(Clusters::<T>::try_get(cluster_id).unwrap().props, ClusterProps { node_provider_auth_contract: user_2 });
	}
