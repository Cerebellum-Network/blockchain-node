use ddc_primitives::{
	...
	NodeParams, NodeStorageUsage, NodeType, NodeUsage, PayoutError, PayoutFingerprintParams,
	PayoutState, StorageNodeMode, StorageNodePubKey, MAX_PAYOUT_BATCH_COUNT, MAX_PAYOUT_BATCH_SIZE,
};
...
impl<T: Config> PayoutProcessor<T> for MockPayoutProcessor {
	...
	fn end_charging_customers(_cluster_id: ClusterId, _era_id: EhdEra) -> DispatchResult {
		unimplemented!()
	}
	...
	fn end_rewarding_providers(_cluster_id: ClusterId, _era_id: EhdEra) -> DispatchResult {
		unimplemented!()
	}

	fn end_payout(_cluster_id: ClusterId, _era_id: EhdEra) -> DispatchResult {
		unimplemented!()
	}
	...
	fn is_customers_charging_finished(_cluster_id: &ClusterId, _era_id: EhdEra) -> bool {
		true
	}

	fn is_providers_rewarding_finished(_cluster_id: &ClusterId, _era_id: EhdEra) -> bool {
		true
	}

	fn get_payout_state(_cluster_id: &ClusterId, _era_id: EhdEra) -> PayoutState {
