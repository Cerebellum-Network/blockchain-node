...
fn test_record_security_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(SecurityAudit::get_security_events_by_type(&SecurityEventType::SuspiciousActivity), vec![0]);
        assert_eq!(SecurityAudit::get_security_events_by_severity(&SecuritySeverity::High), vec![0]);
    });
}
...
fn test_clear_security_events() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(SecurityAudit::clear_security_events(
            RuntimeOrigin::root(),
            vec![0]
        ));
...
fn test_clear_security_events_non_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SecurityAudit::clear_security_events(
                RuntimeOrigin::signed(1),
                vec![0]
            ),
...
fn test_get_security_events_by_filters() {
    new_test_ext().execute_with(|| {
        ...
        let suspicious_events = SecurityAudit::get_security_events_by_type(&SecurityEventType::SuspiciousActivity);
        assert_eq!(suspicious_events, vec![0, 2]);

        let unauthorized_events = SecurityAudit::get_security_events_by_type(&SecurityEventType::UnauthorizedAccess);
        assert_eq!(unauthorized_events, vec![1]);

        // Test filtering by severity
        let high_severity_events = SecurityAudit::get_security_events_by_severity(&SecuritySeverity::High);
        assert_eq!(high_severity_events, vec![0]);

        let critical_events = SecurityAudit::get_security_events_by_severity(&SecuritySeverity::Critical);
        assert_eq!(critical_events, vec![1]);

        let medium_events = SecurityAudit::get_security_events_by_severity(&SecuritySeverity::Medium);
