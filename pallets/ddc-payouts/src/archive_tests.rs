...
use ddc_primitives::{BucketUsage, BucketId};
use sp_io::hashing::blake2_256;
use sp_runtime::Perquintill;

use super::{mock::*, *};

// Wrie simple prf to add number
...
fn begin_billing_report_works() {
	ExtBuilder.build_and_execute(|| {
		...
		System::assert_last_event(Event::PayoutInitialized { cluster_id, era: era.clone() }.into());
		let receipt = crate::PayoutReceipts::<Test>::get(cluster_id, era).unwrap();
		assert_eq!(receipt.state, PayoutState::Initialized);
		//TODO: Ask about follwing tests from @yahor
...
fn begin_charging_customers_works() {
	ExtBuilder.build_and_execute(|| {
		...
		System::assert_last_event(Event::ChargingStarted { cluster_id, era: era.clone() }.into());
		let receipt = crate::PayoutReceipts::<Test>::get(cluster_id, era).unwrap();
		assert_eq!(receipt.state, PayoutState::ChargingCustomers);
		assert_eq!(receipt.charging_max_batch_index, max_charging_batch_index);
	})
}
...
fn send_charging_customers_batch_fails_uninitialised() {
	ExtBuilder.build_and_execute(|| {
		...
		let collector_id = [3u8; 32];
		let payer1_account = AccountId32::new([1; 32]);
		let payer2_account = AccountId32::new([2; 32]);
		let payer1_amount = 100;
		let payer2_amount = 200;
		let payers1 = vec![(payer1_account, payer1_amount)];
		let payers2 = vec![(payer2_account, payer2_amount)];
		let start_date = NaiveDate::from_ymd_opt(2023, 4, 1).unwrap(); // April 1st
		let time = NaiveTime::from_hms_opt(0, 0, 0).unwrap(); // Midnight
		let start_era: i64 =
			DateTime::<Utc>::from_naive_utc_and_offset(start_date.and_time(time), Utc).timestamp();
...
fn end_charging_customers_works_small_usage_1_hour() {
	ExtBuilder.build_and_execute(|| {
		System::set_block_number(1);
		//TODO: What new user is created for every test
		...
		let user6_amount = 100;
		let user7_amount = 100;
		let payers1 = vec![(user6, user6_amount)];
		let start_date = NaiveDate::from_ymd_opt(2023, 4, 1).unwrap(); // April 1st
		let time = NaiveTime::from_hms_opt(0, 0, 0).unwrap(); // Midnight
		let start_era: i64 =
			DateTime::<Utc>::from_naive_utc_and_offset(start_date.and_time(time), Utc).timestamp();
		...
		let receipt_before = crate::PayoutReceipts::<Test>::get(cluster_id, era).unwrap();
		...
		assert_eq!(charge7.puts + charge6.puts, receipt_before.total_customer_charge.puts);
		assert_eq!(charge7.gets + charge6.gets, receipt_before.total_customer_charge.gets);
		assert_eq!(charge7.storage + charge6.storage, receipt_before.total_customer_charge.storage);
		assert_eq!(
			charge7.transfer + charge6.transfer,
			receipt_before.total_customer_charge.transfer
		);
		...
		assert_eq!(
			...
			total_left_from_one * receipt_before.total_customer_charge.transfer
		);
		assert!(report_after.total_customer_charge.transfer > 0);
		assert_eq!(
			report_after.total_customer_charge.storage,
			total_left_from_one * receipt_before.total_customer_charge.storage
		);
		assert!(report_after.total_customer_charge.storage > 0);
		assert_eq!(
			report_after.total_customer_charge.puts,
			total_left_from_one * receipt_before.total_customer_charge.puts
		);
		assert_eq!(
			report_after.total_customer_charge.gets,
			total_left_from_one * receipt_before.total_customer_charge.gets
		);
	})
}
//
