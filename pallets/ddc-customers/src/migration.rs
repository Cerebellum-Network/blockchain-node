...
impl<T: Config> OnRuntimeUpgrade for MigrateToV1<T> {
	...
	#[cfg(feature = "try-runtime")]
	fn post_upgrade(
		(prev_bucket_id, prev_count): (Vec<u8>, Vec<u8>),
	) -> Result<(), TryRuntimeError> {
		let prev_bucket_id: u64 = Decode::decode(&mut prev_bucket_id.as_slice())
			.expect("prev_bucket_id should be something that was generated by pre_upgrade");
		let prev_count: u64 = Decode::decode(&mut prev_count.as_slice())
			.expect("prev_count should be something that was generated by pre_upgrade");
		...
		Buckets::<T>::iter().try_for_each(|(_id, bucket)| -> Result<(), TryRuntimeError> {
			ensure!(
