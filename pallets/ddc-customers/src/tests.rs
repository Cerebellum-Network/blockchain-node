...
fn create_bucket_works() {
	ExtBuilder.build_and_execute(|| {
		...
		// Bucket created
		assert_ok!(DdcCustomers::create_bucket(RuntimeOrigin::signed(1), cluster_id));
...
fn deposit_and_deposit_extra_works() {
	ExtBuilder.build_and_execute(|| {
		...
		// Deposit dust
		assert_noop!(
			DdcCustomers::deposit(RuntimeOrigin::signed(1), 0_u128),
			Error::<Test>::InsufficientDeposit
		);

		// Deposit all tokens fails (should not kill account)
		assert_noop!(
			DdcCustomers::deposit(RuntimeOrigin::signed(1), 100_u128),
			BalancesError::<Test, _>::KeepAlive
		);

		// Deposited
		assert_ok!(DdcCustomers::deposit(RuntimeOrigin::signed(1), 10_u128));
		...
		// Checking that event was emitted
		System::assert_last_event(Event::Deposited(1, 10).into());

		// Deposit should fail when called the second time
		assert_noop!(
			DdcCustomers::deposit(RuntimeOrigin::signed(1), 10_u128),
			Error::<Test>::AlreadyPaired
		);

		// Deposit extra fails if not owner
		assert_noop!(
			DdcCustomers::deposit_extra(RuntimeOrigin::signed(2), 10_u128),
			Error::<Test>::NotOwner
		);

		// Deposited extra
		assert_ok!(DdcCustomers::deposit_extra(RuntimeOrigin::signed(1), 20_u128));
		...
		// Checking that event was emitted
		System::assert_last_event(Event::Deposited(1, 20).into());
	})
}
...
fn unlock_and_withdraw_deposit_works() {
	ExtBuilder.build_and_execute(|| {
		...
		// Deposited
		assert_ok!(DdcCustomers::deposit(RuntimeOrigin::signed(1), 35_u128));
		// So there is always positive balance within pallet
		assert_ok!(DdcCustomers::deposit(RuntimeOrigin::signed(2), 10_u128));

		// Unlock chunk
		assert_ok!(DdcCustomers::unlock_deposit(RuntimeOrigin::signed(1), 1_u128));
		...
		assert_noop!(
			DdcCustomers::unlock_deposit(RuntimeOrigin::signed(1), 1_u128),
		...
		assert_ok!(DdcCustomers::unlock_deposit(RuntimeOrigin::signed(1), 3_u128));
