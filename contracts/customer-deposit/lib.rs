...
mod customer_deposit {
    ...
    impl CustomerDepositContract {
        ...
        pub fn deposit(&mut self) -> Result<(), Error> {
            ...
            if let Some(mut ledger) = self.balances.get(&owner) {
                ...
                ledger.active = ledger.active.checked_add(value).ok_or(Error::ArithmeticOverflow)?;
        ...
        pub fn deposit_for(
            ...
            if !self.balances.contains(&owner) {
                // New ledger - no need for existential deposit check since contract holds all tokens
                let ledger = Ledger {
                    owner: owner,
                    total: value,
                    active: value,
        ...
        pub fn unlock_deposit(&mut self, value: Balance) -> Result<(), Error> {
            ...
            let unlock_block = current_block.checked_add(UNLOCK_DELAY_BLOCKS)
                .ok_or(Error::ArithmeticOverflow)?;
        
            // Merge chunks unlocking at the same block (like pallet)
            if let Some(chunk) = ledger.unlocking.last_mut().filter(|chunk| chunk.block == unlock_block) {
        ...
        pub fn withdraw_unlocked(&mut self) -> Result<(), Error> {
            ...
            let withdrawn = old_total.checked_sub(ledger.total).ok_or(Error::ArithmeticUnderflow)?;
            if withdrawn == 0 {
                return Err(Error::NothingToWithdraw);
            }

            self.env().transfer(owner, withdrawn)
    ...
    impl cere_runtime_traits::DdcPayouts for CustomerDepositContract {
        #[ink(message)]
        fn charge(&mut self, _vault: AccountId, _batch: Vec<(AccountId, u128)>) -> Result<Vec<(AccountId, u128)>, ()> {
            unimplemented!()
        }
    }
...
mod tests {
    use super::*;
    use ink::env::test;
    use crate::customer_deposit::{MIN_EXISTENTIAL_DEPOSIT, UNLOCK_DELAY_BLOCKS, ClusterId, CustomerDepositContract};
    ...
    fn test_deposit_for_dust_fails() {
        ...
        // Deposit fails (dust)
        assert_eq!(
            contract.deposit_for(accounts.bob).unwrap_err(),
            Error::InsufficientDeposit
        );
    }
    ...
    fn test_withdraw_unlocked_early_fails() {
        ...
        assert_eq!(
            contract.withdraw_unlocked().unwrap_err(),
